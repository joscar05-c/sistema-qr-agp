...
<div class="p-4 space-y-6">
    ...
    <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 text-center relative overflow-hidden">
        <div class="absolute top-0 left-0 w-full h-1" style="background-color: {{ $subArea->area->color ?? '#1e3a8a' }}"></div>
        ...
        @if($subArea->oficina || $subArea->area->piso)
            <div class="mt-4 inline-flex items-center gap-2 bg-slate-100 px-4 py-2 rounded-full text-sm text-gray-700 font-medium">
    ...
    <div>
        ...
        <div class="space-y-3">
            @forelse($subArea->especialistas as $especialista)
                <a href="{{ url('/v/' . $especialista->slug) }}" class="block bg-white p-4 rounded-xl shadow-sm border border-gray-200 hover:border-blue-400 hover:shadow-md transition group relative overflow-hidden">
                    <div class="flex items-center gap-4 relative z-10">
                        <div class="relative shrink-0">
                            <img src="{{ $especialista->image_url }}" alt="{{ $especialista->nombre_completo }}" class="w-14 h-14 rounded-full object-cover border border-gray-200 group-hover:border-blue-300 transition">
                        </div>

                        <div class="flex-1 min-w-0">
                            <h4 class="text-base font-bold text-gray-900 truncate">{{ $especialista->nombre_completo }}</h4>
                            <p class="text-sm text-gray-500 truncate">{{ $especialista->cargo ?? 'Especialista' }}</p>
                        </div>

                        <div class="shrink-0 text-gray-300 group-hover:text-blue-600 transition transform group-hover:translate-x-1">
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
